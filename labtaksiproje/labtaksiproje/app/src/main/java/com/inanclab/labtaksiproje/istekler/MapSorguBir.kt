...
object MapSorguBir {
    ...
    lateinit var bolge : String

    //assets doyasına erişmek için fonksiyon
    fun kimlikBilgileriniAl(context: Context){
        MapSorguBir.kimlikbilgileri = GoogleCredentials.fromStream(context.assets.open("airy-adapter-311618-cd7d649dde7e.json"))
    }
    ...
    suspend fun sqlsorgusu() {
        ...
        for (row: FieldValueList in result.iterateAll()) {
            ...
            CoroutineScope(Dispatchers.Default).launch {
                KonumIstek.getLocationInfo(bolge)
                KonumIstek.getLatLong(KonumIstek.json)
            }
        }
